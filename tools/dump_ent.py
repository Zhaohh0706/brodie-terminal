...
unm={a:[round(dep[a]/1e18,2),round(v/1e18,2)] for a,v in ent.items() if mr.get(a.lower(),0)==0}
res={"generated":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"block":latest,
     ...
     "unmintedCount":len(unm),"unmintedTotal":round(sum(v[1] for v in unm.values()),2),
     "unminted":dict(sorted(unm.items(),key=lambda x:-x[1][1]))}
json.dump(res,open("unminted.json","w"),indent=1)
